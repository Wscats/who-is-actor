...
于是 who-is-actor 诞生了 —— 一个**零依赖、零脚本**的 Git 仓库开发者画像分析 Skill。它只用原生 `git` 命令采集数据（所有输入经过严格校验，不采集邮箱），然后交给 AI 来解读，生成一份毫不留情的六维度体检报告。
...
| 分析维度 | **6 个**（含参与度指数） | 通常 3-5 个 |
| 输出风格 | 严肃犀利的个性化点评 | 中性数据报告 |
| 可定制性 | 自然语言调整 | 修改代码/配置 |
| 安全性 | 输入白名单校验 + 不采集邮箱 | 依赖库自行处理 |
