...
| 61-80 | 🤔 低可见活动 | 建议了解是否有未被 Git 记录的非代码贡献 |
| 81-100 | ❓ 极低可见活动 | 有多种可能原因，请勿仅凭此下结论 |
...
综合评分 = 提交习惯 × 15% + 工作习惯 × 15% + 研发效率 × 25%
         + 代码风格 × 15% + 代码质量 × 20% + (100 - 可见活动指数) / 10 × 10%
```

该评分仅反映仓库中可观测到的协作表现维度加权后的仓库级信号。**严禁**被解读为个人能力评分。

## 生成的报告长什么样？

### 总览表

| 贡献者 | 提交数 | 增/删行数 | 日均提交 | 周末% | 深夜% | Bug Fix% | 流失率 | 可见活动 | 综合评分 |
|--------|--------|-----------|----------|-------|-------|----------|--------|---------|----------|
...
> 以上姓名仅作为 Git 提交归属标签。报告描述的是仓库模式，不是个人能力评判。

### 每位贡献者的仓库可见表现

每位贡献者会拿到：
1. **数据仪表盘** — 六维度关键指标（均为仓库可见表现）
2. **仓库级点评** — 描述该区域代码表现出的模式，不讲“谁怎么了”
3. **面向流程的改进建议** — 针对可见信号提供可供讨论的下一步
4. **六维雷达评分**— 维度 1-10，均为仓库表现信号强度
5. **一句话总结** — 描述该区域代码以什么节奏被提交，不会对个人下论断
## 写在最后：数据不是审判
